...
例子 （如果改变已经发生了，你再对Promise对象添加回调函数，也会立即得到这个结果。）
...
console.log((new Promise(function (resolve, reject) {
    ...
    setTimeout(() => {
        console.log(5)
        resolve(new Promise(function (resolve1, reject) {
            resolve1(11)
        }))
        console.log(6)
    }, 3000)
...
6.Promise.all() // // 所有 new Promise 实例 是 resolve； Promise 实例 全部为 resolve, 或者 返回第一个是 reject 的 Promise 实例
...
7.Promise.race() // 返回第一个 Promise 实例的状态 
...
8.Promise.allSettled() // 等待所有 Promise 实例返回（无论 resolve or reject）， 返回结果 Promise 的实例，且 status 为 resolve
...
const resolved = Promise.resolve(42);
const rejected = Promise.reject(-1);
const allSettledPromise = Promise.allSettled([resolved, rejected]).then(res => {
  console.log(res) // [{status: "fulfilled", value: 42}, {status: "rejected", reason: -1}]
});


9.Promise.any()   // 任意一个 new Promise 实例 是 resolve
...
13.Promise.try() // promise.catch()捕获所有同步和异步的错误。
