...
export default class Comp extends React.PureComponent {
    ...
    renderTitle = () =>{
        return <h1>Iterator 和 for...of 循环</h1>
    }
...
原因在于，这些数据结构原生部署了Symbol.iterator属性（详见下文），另外一些数据结构没有（比如对象）。凡是部署了Symbol.iterator属性的数据结构，就称为部署了遍历器接口。调用这个接口，就会返回一个遍历器对象。
...
// var it2 = new makeIterator(['a', 'b']);
// function makeIterator(array) {
//   this.nextIndex = 0;
//   return {
//     next: () => this.nextIndex < array.length ? { value: array[this.nextIndex++], done: false } : { value: undefined, done: true }
//   };
// }
...
// var it3 = new makeIterator(['a', 'b']);
// function makeIterator(array) {
//   this.nextIndex = 0;
//   return {
//     next: (() => return this.nextIndex < array.length ? { value: array[this.nextIndex++], done: false } : { value: undefined, done: true }).bind(this)
//   };
// }
...
//   };
// }
//  console.log(it4.next()) // { value: "a", done: false }
//  console.log(it4.next()) // { value: "b", done: false }
//  console.log(it4.next()) // { value: undefined, done: true }


使用 TypeScript 的写法，遍历器接口（Iterable）、指针对象（Iterator）和next方法返回值的规格可以描述如下。
...
原生具备 Iterator 接口的数据结构如下。 [Symbol.iterator]: 属性（function） (无Object) 可执行 for...of 方法
Array
Map
Set
String
...
本质上，遍历器是一种线性处理，对于任何非线性的数据结构，部署遍历器接口，就等于部署一种线性转换。(线性: 0,1,2,3,4,5,6.....)
